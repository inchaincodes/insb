...
var DbOpt = {
    ...
    pagination : function(obj,req,res,conditions){
        ...
        if (!params.query.currentPage || !params.query.limit) {
...
//    根据条件查询记录(如果有条件传递，则按条件查询)
        ...
        if(conditions && conditions.length > 1){
            ...
            queryCount=obj.find().or(conditions).count();
        }
        else if(conditions){
            query=obj.find(conditions);
            queryCount=obj.find(conditions).count();
        }
        else{
            query=obj.find({});
            queryCount=obj.find().count();
        }

        var resultNum = queryCount.exec();
        ...
        var resultList = query.skip(startNum).limit(limit).exec();
        pageInfo = {
    ...
    getPaginationResult : function(obj,req,res,q,filed){// 通用查询，带分页，注意参数传递格式,filed为指定字段
        ...
        if(q && q.length > 1){ // 多条件只要其中一条符合
            // if ( obj==Content ) {
            //     resultList = obj.find({'state':true}).populate('author','name userName').or(q,filed).sort(sq).skip(startNum).limit(limit);
            // }  else {
            ...
            // if ( obj==Content ) {
            //     resultList = obj.find(q,filed).populate('author','name userName').sort(sq).skip(startNum).limit(limit);
           // } else {
